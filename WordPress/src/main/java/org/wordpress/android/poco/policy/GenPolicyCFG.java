...
public class GenPolicyCFG {
	...
	private static void add2CFG(CFG cfg, Set<CFG> flagNodes, Set<CFG> pocoMtdNodes) {
		...
		if( ParsFlgConsts.IS_STATEMENT_FLAG(sig) ) {
			...
			if(sig.startsWith("edu.usfcse.poco")) {
				if(!sig.equals("edu.usfcse.poco.policy.Policy.setOutput(Event)") && !sig.startsWith(_currPolicyName))
	...
	private static void add2CFG(CFG cfg) {
		if (_currCFG.getEvent() == null || _stack4genCFG.size() == 0) {
			...
			switch (cfg.getEvent().getSig()) {
				case ParsFlgConsts.ELSE_BRANCH:         addElseBranching(cfg,0);	    break;
				case ParsFlgConsts.ENDOFIF:	            addEndIfNode(cfg,0);		    break;
				case ParsFlgConsts.CATCH_BLOCK:	        handleCatchNode(cfg);  		    break;
				case ParsFlgConsts.ENDTRY:              handleEndTryNode(cfg);  	    break;
                case ParsFlgConsts.END_ENHANCED_FOR:    handleEnhancedForBlock(cfg, 0); break;
				default:
    ...
    private static void handleEnhancedForBlock(CFG cfg, int offset) {
        ...
        int index4Else = locateMatchingEnhancedFor(offset);
        if(index4Else != -1)  {
	...
	private static void handleCatchNode(CFG cfg) {
        ...
        int index = locateMatchingTryBlock(0);
        // step 1.1 always should be able to locate the matching try block statements
		if(index == -1) {
			System.err.println("Something went wrong when analyzing the " + _currMtdName + " method of the " + _currPolicyName + " policy!");
			System.exit(index);
		}

		// step 1.2 if there exists nested try-statements,  actions w/i those try-statements
	...
	private static int locateMatchingTryBlock(int offset) {
		return locateMatchingFlag(ParsFlgConsts.TRY, offset, ParsFlgConsts.ENDTRY, ParsFlgConsts.TRY);
	}
    ...
    private static int locateMatchingEnhancedFor(int offset) {
        return locateMatchingFlag(ParsFlgConsts.ENHANCED_FOR_BLOCK, offset, ParsFlgConsts.END_ENHANCED_FOR, ParsFlgConsts.ENHANCED_FOR);
    }
